...
import hubconf
from quant import *
from data.mnist import build_mnist_data
...
@torch.no_grad()
def validate_model(val_loader, model, device='cuda', print_freq=100):
    if device is None:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ptm', default='./', type=str, help='path to pretrained mobilenetv2')

    args = parser.parse_args()

    seed_all(args.seed)
    # build imagenet data loader
    train_loader, test_loader = build_mnist_data(batch_size=args.batch_size, workers=args.workers,
                                                    data_path=args.data_path)

    # load model
    cnn = eval("hubconf.{a}(pretrained=True)".format(a=args.arch))
    cnn.cuda()
    cnn.eval()
    ...
    # test_loader = get_train_samples(test_loader, num_samples=args.num_samples)
    ...
    if args.test_before_calibration:
        a = validate_model(test_loader, qnn)
        print('Quantized accuracy before brecq: {}'.format(a))
    ...
    #_ = qnn(cali_data[:64].to(device))
    b = validate_model(test_loader, qnn)
    print('Weight quantization accuracy: {}'.format(b))
    ...
    if args.act_quant:
        ...
        c = validate_model(test_loader, qnn)
        print('Quantized accuracy before brecq: {}'.format(a))
        print('Weight quantization accuracy: {}'.format(b))
